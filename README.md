...
<h2>Task 1: Spatial Feature Learning (Static CNN)</h2>
**Deadline:** 2nd March
...
- **Goal:** Train a custom CNN model to recognize the 26 static ASL letters from individual images.
- **Dataset:** Use SignAlphaSet.zip.
- **Action:** Train a custom CNN that classifies the image into one of the 26 letters. Feel free to use any existing architectures, but code it from scratch using PyTorch. Experiment with different kernels, block sizes, and other parameters. Also ensure that your model doesn’t overfit — keep a train/validation split and evaluate accordingly. This will be the base of the entire model, so aim for strong accuracy.
- **Output:** Save the weights and push that file along with your model code to your branch.
